  ...
  colorSetup: ColorSetup,
    ...
    if (fd.radiusMetric && fd.radiusMetric instanceof Object) {
        radiusData = data.map((d) => {
            return Number(d[(fd.radiusMetric as any).label]);
        });
        radiusLimits = [Math.min(...radiusData), Math.max(...radiusData)]
        maxRadius = Number(fd['radiusMax'])
        minRadius = Number(fd['radiusMin'])
    }

    return geo.map((feature, index) => {
        const {coordinates} = feature.geometry;
        if (fd.markerIconUrl && fd.iconUrlColumn && fd.iconUrlColumn.length > 0) {
            const markerIcon = new L.Icon({
                iconUrl: (data[index]?.[fd.iconUrlColumn as string] as string) || 'data/black-placeholder-variant.png',
                iconSize: [fd.markerSize, fd.markerSize], // Größe des Icons
            })
            return (
                <Marker
