...
const getLegendColor = (colorSetup: ColorSetup, value: any) => {
...
export const Legend = ({colorSetup, legendTitle}: {colorSetup: ColorSetup; legendTitle?: string}) => {
    const nColors = 100
    const getColorScaleLegend = () => {
        let colors = []
        const legendItems = [];
        if (colorSetup.dataType === 'continuous') {
            let data: (string | null)[] = [colorSetup.colorLimits[0].toFixed(1)]
            ...
            let data = [...colorSetup.colorData].sort((a: any, b: any) => a - b)
            colors = data.map((value: any) => {
                return getLegendColor(colorSetup, value);
            });
            ...
            for (let key of Object.keys(colorSetup.colorMap)) {
                ...
                legendItems.push(
                    <li key={key}><span style={{background: background}}></span>
